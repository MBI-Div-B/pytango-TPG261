...
class TPG26xTango(Device):
    ...
    pressure_minute = attribute(
        label="Pressure every second",
        dtype=[
            float,
        ],
        max_dim_x=60,
    ...
    pressure_hour = attribute(
        label="Pressure every minute",
        dtype=[
            float,
        ],
        max_dim_x=60,
    ...
    pressure_day = attribute(
        label="Pressure every hour",
        dtype=[
            float,
        ],
        max_dim_x=24,
    ...
    pressure_month = attribute(
        label="Pressure every day",
        dtype=[
            float,
        ],
        max_dim_x=30,
    ...
    def init_device(self):
        ...
        if self.Type == "TPG261":
            self.controller = TPG261()
            #self.connection = self.controller.serial
            if True: #self.connection.isOpen():
                self.set_state(DevState.ON)
                self.info_stream(
                    "TPG261 initialised on port %s." % 'irgendwo' # self.controller.address
                )
        elif self.Type == "TPG262":
            self.controller = TPG262()
            #self.connection = self.controller.serial
            if True: # self.connection.isOpen():
                self.set_state(DevState.ON)
                self.info_stream(
                    "TPG262 initialised on port %s." % 'irgendwo' # self.controller.address
                )
        else:
            pass
    ...
    def read_pressure(self):
        ...
        if new_time - self._time[1] >= (60):
            ...
            self.info_stream(
                "Hour array was updated. New value is %.6f, new time is %d",
                self._hour_array[-1],
                new_time,
            )
        ...
        if new_time - self._time[2] >= (60 * 60):
            ...
            self.info_stream(
                "Day array was updated. New value is %.6f, new time is %d",
                self._day_array[-1],
                new_time,
            )
        ...
        if new_time - self._time[3] >= (60 * 60 * 24):
            ...
            self.info_stream(
                "Month array was updated. New value is %.6f, new time is %d",
                self._month_array[-1],
                new_time,
            )
